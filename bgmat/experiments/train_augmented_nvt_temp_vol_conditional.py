...
def _get_loss(
    ...
    # Different betas for all samples
    betas = 1. / (state.kb * base_temps).reshape(-1)
    ...
    samples_mapped, log_det = flow.forward_and_log_det(base_samples, lattice=new_lattice, box_length=new_box, context=base_temps.reshape(-1,1,1))
...
def create_temp_base(config):
    return config.model.kwargs.base_temp["constructor"](
        **config.model.kwargs.base_temp.kwargs
    )
...
def train(config, system, project_name=None, run_name=None, job_id=None, model_hash=None):
    ...
    def loss_fn():
        ...
        base_temp = create_temp_base(config)
        base_density = create_vol_base(config)
        flow = create_flow(config)
        
        rng_key = hk.next_rng_key()
        rng_key, temp_key, vol_key = jax.random.split(rng_key, 3)
        num_samples = config.train.batch_size // config.train.every_k_schedule
        base_temps = base_temp.sample(
        seed=temp_key,sample_shape=(num_samples,1)
    )
