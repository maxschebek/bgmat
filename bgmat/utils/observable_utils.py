...
def squared_distance_pbc(coordinate_deltas: Array, box_length: Array) -> Array:
    ...
    coordinate_deltas_pbc = coordinate_deltas - box_length * jnp.round(
...
def difference_pbc(
    coordinates_a: Array, coordinates_b: Array, box_length: Array
) -> Array:
    """Computes pairwise distance vectors obeying periodic boundary conditions.
    ...
    Returns:
      Array with shape [..., num_particles, num_particles, dim], the pairwise
      difference vectors with respect to periodic boundary conditions.
    """
    deltas = coordinates_a - coordinates_b
    return deltas - box_length * jnp.round(deltas / box_length)
...
def distance_matrix_pbc(
    ...
    Returns:
      ...
      difference vectors with respect to periodic boundary conditions.
    """
    differences = difference_pbc(
        coordinates_a[:, None], coordinates_b[None, :], box_length
    )
    return jnp.linalg.norm(differences, axis=-1)


def difference(coordinates_a: Array, coordinates_b: Array) -> Array:
    """Computes distance vectors between two arrays without periodic boundary conditions.
    ...
    Returns:
      Array with shape [..., num_particles, dim], the difference vectors.
    """
    deltas = coordinates_a - coordinates_b
    return deltas
...
def radial_distribution_function(
    coordinates: Array, box_length: Array, num_bins: int = 300
) -> Array:
    ...
    box_volume = jnp.prod(box_length)
    ...
    hist, bins = jnp.histogram(dr, bins=num_bins, range=(0, min_box_length / 2.0))
...
def compute_histogram(
    ...
    hist, bins = jnp.histogram(data, bins=num_bins, density=density, range=data_range)
    centers = (bins[:-1] + bins[1:]) / 2
    return jnp.column_stack((centers, hist))


def free_energy_fep(forward: Array, beta: float) -> Array:
...
@jax.jit
def free_energy_fep_running(forward: Array, beta: float, num_evals: int = 10) -> Array:
...
def _compute_importance_weights(
    model_log_probs: Array, target_log_probs: Array
) -> Array:
...
@jax.jit
def compute_importance_estimate(
    x: Array, model_log_probs: Array, target_log_probs: Array
) -> Array:
    """Return an importance sampling estimate of the mean value of `x`."""
    weights = _compute_importance_weights(model_log_probs, target_log_probs)
    if weights.shape != x.shape:
        raise ValueError(
            "The shape of the importance sampling weights "
